...
def get_top_10_similar(movie_id, use_overview_for_similarity=False): 
    ...
    top_ten_similar = get_k_recommendations_based_on_soup(title , 10)
    top_ten_similar = format_data_objects(get_movies_from_ids(list(top_ten_similar.to_dict().keys())))
...
def get_rating(user_id):
    ...
    movie = prediction.apply(get_movie_poster_and_trailer, axis=1, get_trailer=True)
